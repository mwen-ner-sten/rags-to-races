  ...
  return {
    ...
    applyTickResult: (partsFound: ScavengedPart[], scrapsEarned: number, repEarned: number, vehicleWear?: number, vehicleRepair?: number) => {
      set((s: GameState) => {
        ...
        const nextInventory = [...s.inventory, ...partsFound];
        const nextRep = s.repPoints + repEarned;
        let nextFeatures = s.unlockedFeatureIds;
        let nextUnlockEvents = s.unlockEvents;

        if (!nextFeatures.includes("garage") && nextInventory.length > 0) {
          nextFeatures = [...nextFeatures, "garage"];
          nextUnlockEvents = [...nextUnlockEvents, "Garage Unlocked! Time to bolt parts together."];
        }

        if (!nextFeatures.includes("community") && nextRep >= 5) {
          nextFeatures = [...nextFeatures, "community"];
          nextUnlockEvents = [...nextUnlockEvents, "Community Unlocked! Rivals and fans are starting to notice."];
        }
        ...
        return {
          ...
          lifetimeRaces: newLifetimeRaces,
          fatigue: newFatigue,
          unlockedFeatureIds: nextFeatures,
