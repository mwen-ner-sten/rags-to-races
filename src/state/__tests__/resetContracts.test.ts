...
describe("four-layer reset contracts", () => {
  ...
  it("Team clears station equipment, non-lead crew, vehicles, and Garage Philosophy but preserves discoveries", () => {
    ...
    expect(state.garage).toEqual([]); expect(state.crewRoster).toEqual([expect.objectContaining({ id: "c", role: "driver" })]); expect(state.unlockedPlaystyleNodes).toEqual([]); expect(state.stationEquipmentInventory).toEqual([]); expect(state.discoveredBlueprintIds).toEqual(["known"]);
  });
