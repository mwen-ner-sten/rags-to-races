...
export const UPGRADE_DEFINITIONS: UpgradeDefinition[] = [
  ...
  // ── Efficiency (Tick Speed) ──────────────────────────────────────────────────
  {
    id: "tick_accelerator",
    name: "Speed Dial",
    description: "Reduces the global tick interval by 4s per level (max 7 levels: 30s → 2s).",
    category: "maintenance",
    maxLevel: 7,
    baseCost: 2500,
    costScaling: 2.5,
  ...
  },
  {
    ...
    description: "Further reduces tick interval by 0.9s per level (2s → 0.2s min). Requires Speed Dial maxed.",
    category: "maintenance",
    maxLevel: 2,
    baseCost: 7500,
    costScaling: 2.0,
    effect: { type: "tick_speed_reduction_ms", valuePerLevel: 900 },
