...
function isStepConditionMet(
  ...
  switch (step) {
    ...
    case 11: return !state.isRacing && state.raceHistory.length > 0;
    case 13: {
...
export default function TutorialOverlay({ activeTab }: Props) {
  ...
  if (tutorialStep === 12) {
    ...
    // entry after a reload; the generic fallback also keeps damaged/legacy
  ...
  if ((showCard || showGoalIntro) && anchorRect) {
    ...
    cardStyle = {
      ...
      top,
      width: cardW,
      maxHeight: viewW < 640 ? "calc(100vh - 88px)" : "calc(100vh - 32px)",
      overflowY: "auto",
      zIndex: 10000,
    };
