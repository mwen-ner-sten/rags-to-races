...
import { PART_DEFINITIONS, CONDITIONS, type PartCondition } from "@/data/parts";
...
import { getGearById, getGearForSlot, type GearSlot } from "@/data/gear";
...
const LEGACY_SLOT_TO_STATION: Record<GearSlot, GarageStationSlot> = {
  head: "diagnostics",
  body: "lift",
  hands: "workbench",
  feet: "logistics",
  tool: "fabrication",
  accessory: "pit_equipment",
};

const STATION_TO_LEGACY_SLOT = Object.fromEntries(
  Object.entries(LEGACY_SLOT_TO_STATION).map(([slot, station]) => [station, slot]),
) as Record<GarageStationSlot, GearSlot>;

const TIER_RARITY: GearRarity[] = ["common", "uncommon", "rare", "epic", "legendary"];
      ...
      {parts.length === 0 ? <Empty text="No parts yet. Scavenge first, then return here to engineer them." /> : (
        <div className="grid grid-cols-1 gap-2 lg:grid-cols-2">
          {parts.map((part) => {
            const def = PART_DEFINITIONS.find((item) => item.id === part.definitionId);
            if (!def) return null;
            const installed = active?.parts[def.category]?.part;
            const compare = installed ? def.basePower - (PART_DEFINITIONS.find((item) => item.id === installed.definitionId)?.basePower ?? 0) : null;
              ...
              <article key={part.id} className="rounded-lg border p-3" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}>
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs" style={{ color: "var(--text-muted)" }}>{def.category} · Power {def.basePower} · Reliability {def.baseReliability} · {def.baseWeight}kg</p>
                    <p className="mt-1 text-xs" style={{ color: compare === null ? "var(--text-muted)" : compare >= 0 ? "var(--success)" : "var(--danger)" }}>
                      {compare === null ? "No matching part installed on the focus vehicle" : `${compare >= 0 ? "+" : ""}${compare} base power vs installed`}
                    </p>
                  </div>
                </div>
...
function StationEquipment() {
  const equippedGear = useGameStore((s) => s.equippedGear); const owned = useGameStore((s) => s.ownedGearIds);
  const loot = useGameStore((s) => s.lootGearInventory); const equippedLoot = useGameStore((s) => s.equippedLootGear);
  const equipStatic = useGameStore((s) => s.equipGear); const equipLoot = useGameStore((s) => s.equipLootGear); const enhance = useGameStore((s) => s.enhanceLootGear);
  return <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">{GARAGE_STATIONS.map((station) => {
    const slot = STATION_TO_LEGACY_SLOT[station.id]; const currentLoot = loot.find((item) => item.id === equippedLoot[slot]); const currentStatic = getGearById(equippedGear[slot]);
    const options = loot.filter((item) => item.slot === slot); const staticOptions = getGearForSlot(slot).filter((item) => owned.includes(item.id));
    const rarity = currentLoot?.rarity ?? TIER_RARITY[Math.min(currentStatic?.tier ?? 0, 4)];
    return <article key={station.id} className="rounded-lg border p-3" style={{ borderColor: "var(--panel-border)", background: "var(--panel-bg)" }}><div className="flex gap-3"><ComposedEquipmentImage variant={{ slot: station.id, rarity, affixes: STATION_AFFIXES[station.id] }} size={64} /><div><strong style={{ color: "var(--text-white)" }}>{station.name}</strong><p className="text-xs" style={{ color: "var(--text-muted)" }}>{station.description}</p><p className="mt-1 text-xs" style={{ color: "var(--accent)" }}>{currentLoot ? `${currentLoot.name} +${currentLoot.enhancementLevel}` : currentStatic?.name}</p></div></div>
      <div className="mt-3 flex flex-wrap gap-2">{staticOptions.map((item) => <Action key={item.id} onClick={() => equipStatic(item.id)}>{item.name}</Action>)}{options.map((item) => <span key={item.id} className="inline-flex gap-1"><Action onClick={() => equipLoot(item.id)}>{item.name}</Action><Action onClick={() => enhance(item.id)}>+{item.enhancementLevel}</Action></span>)}</div></article>;
  })}</div>;
}
