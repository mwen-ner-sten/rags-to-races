...
export default function Home() {
  ...
  const [pendingTabUnlock, setPendingTabUnlock] = useState<TabId | null>(null);
  ...
  const handleSetActiveTab = (tab: TabId): void => {
    if (unlockedFeatureIds.includes(tab) || tab === "dev") {
      setActiveTab(tab);
      setLockMessage(null);
      setPendingTabUnlock(null);
      return;
    }
    setPendingTabUnlock(tab);
    setLockMessage(`${tab.toUpperCase()} is locked — ${FEATURE_REQUIREMENTS[tab]}`);
  };

  useEffect(() => {
    if (!pendingTabUnlock) return;
    if (!unlockedFeatureIds.includes(pendingTabUnlock)) return;
    setLockMessage(`${pendingTabUnlock.toUpperCase()} is now unlocked. Hit the tab when you're ready.`);
    setPendingTabUnlock(null);
  }, [pendingTabUnlock, unlockedFeatureIds]);
  ...
  return (
    <>
      <ToastContainer />
      {lockMessage && <div className="pointer-events-none fixed inset-x-0 top-0 z-40 h-20 bg-black/30" />}
      <ThemeShell activeTab={activeTab} setActiveTab={handleSetActiveTab}>
        {lockMessage && (
          <div className="mb-4 rounded border border-amber-500/50 bg-amber-500/10 px-3 py-2 text-sm text-amber-200 shadow-lg shadow-black/30">
            {lockMessage}
          </div>
        )}
